...
abstract class InFormCacheableExpr : XPathAnalyzable {
    ...
    fun relevantContextNodesAreCacheable(ec: EvaluationContext): Boolean {
        if (!cacheState.computedContextTypes) {
            ...
            cacheState.contextRefIsRelevant =
                relevantContextTypes?.contains(TreeReference.CONTEXT_INHERITED) ?: false
            cacheState.originalContextRefIsRelevant =
                relevantContextTypes?.contains(TreeReference.CONTEXT_ORIGINAL) ?: false
            cacheState.computedContextTypes = true
        }
        return !(cacheState.contextRefIsRelevant &&
                ec.contextRef != null && contextRefIsUncacheableInForm(ec.contextRef!!)) &&
                !(cacheState.originalContextRefIsRelevant &&
                        ec.getOriginalContext() != null && contextRefIsUncacheableInForm(ec.getOriginalContext()!!))
    }
    ...
    companion object {
        ...
        fun referencesMainFormInstance(
            expr: XPathAnalyzable,
        ...
        @JvmStatic
        @Throws(AnalysisInvalidException::class)
        fun containsUncacheableSubExpression(
            expr: XPathAnalyzable,
            ec: EvaluationContext
        ): Boolean {
