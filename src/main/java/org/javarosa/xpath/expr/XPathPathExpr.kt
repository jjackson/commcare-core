...
class XPathPathExpr : XPathExpression {
    ...
    fun getReference(): TreeReference {
        cachedReference?.let { return it }

        val ref: TreeReference
        val parentsAllowed: Boolean
                    ...
                    if (func.name == "instance") {
                        ...
                        if (func.args.size != 1) {
                            throw XPathUnsupportedException(
                                "instance() function used with ${func.args.size} arguements. Expecting 1 arguement"
                            )
                        }
        ...
        var mutableParentsAllowed = parentsAllowed
        for (step in steps) {
            if (step.axis == XPathStep.AXIS_SELF) {
                ...
                if (!mutableParentsAllowed || step.test != XPathStep.TEST_TYPE_NODE) {
                    throw XPathUnsupportedException(otherStepMessage)
                } else {
    ...
    override fun evalRaw(m: DataInstance<*>?, ec: EvaluationContext): XPathNodeset {
        var model: DataInstance<*>? = m
        ...
        if (genericRef.getContextType() == TreeReference.CONTEXT_ORIGINAL) {
            ...
            ref = genericRef.contextualize(ec.contextRef!!)!!
        }
        ...
        if (ref.getInstanceName() != null && ref.isAbsolute) {
            val nonMain = ec.getInstance(ref.getInstanceName())
            if (nonMain != null) {
                model = nonMain
                ...
                val args = arrayOf<Any?>(ref.getInstanceName(), ref.toString(true), ec.contextRef)
            ...
            model = ec.getMainInstance()

            if (model == null) {
                val refStr = ref.toString(true)
                throw XPathException(
                    "Cannot evaluate the reference [$refStr] in the current evaluation context. No default instance has been declared!"
                )
            }
        }
        ...
        if (!templatePathChecked && ref.isAbsolute && !model!!.hasTemplatePath(ref)) {
        ...
        return XPathLazyNodeset(ref, model, ec)
    }
    ...
    override fun equals(other: Any?): Boolean {
        if (other is XPathPathExpr) {
            ...
            if (initContext != other.initContext || steps.size != other.steps.size) {
                return false
            }

            @Suppress("UNCHECKED_CAST")
            return ExtUtil.arrayEquals(steps as Array<Any?>, other.steps as Array<Any?>, false) &&
                    (initContext != INIT_CONTEXT_EXPR || filtExpr == other.filtExpr)
        } else {
            return false
        }
    }
    ...
    fun matches(o: XPathExpression): Boolean {
        if (o is XPathPathExpr) {
            ...
            for (i in steps.indices) {
                if (!steps[i].matchesWith(o.steps[i])) {
                    return false
                }
            }
    ...
    override fun pivot(
        ...
        return if (ref == sentinal || ref.getRefLevel() == 0) {
            sentinal
        } else {
            ...
            for (i in 0 until ref.size()) {
                if (ref.getPredicate(i) != null && ref.getPredicate(i)!!.size > 0) {
                    throw UnpivotableExpressionException(
                        "Can't pivot filtered treereferences. Ref: ${ref.toString(true)} has predicates."
                    )
                }
            }
            this.eval(model, evalContext)
        }
    }

    override fun toPrettyString(): String {
        return try {
            getReference().toString(true)
        } catch (e: Exception) {
            toDebugString()
        }
    }
    ...
    companion object {
        ...
        fun getRefValue(model: DataInstance<*>, ec: EvaluationContext, ref: TreeReference): Any {
            if (ec.isConstraint && ref == ec.contextRef) {
                ...
                val node: AbstractTreeElement? = model.resolveReference(ref, ec)
                if (node == null) {
        ...
        fun unpackValue(`val`: IAnswerData?): Any {
            return when {
                `val` == null -> ""
                `val` is UncastData -> `val`.value!!
                `val` is IntegerData -> (`val`.getValue() as Int).toDouble()
                `val` is LongData -> (`val`.getValue() as Long).toDouble()
                `val` is DecimalData -> `val`.getValue()
                `val` is StringData -> `val`.getValue()!!
                `val` is SelectOneData -> (`val`.getValue() as Selection).getValue()
                `val` is SelectMultiData -> XFormAnswerDataSerializer().serializeAnswerData(`val`)
                `val` is DateData -> `val`.getValue()
                `val` is DateTimeData -> `val`.getValue()
                `val` is BooleanData -> `val`.getValue()
                `val` is GeoPointData -> `val`.uncast().getString()!!
                else -> {
                    println("warning: unrecognized data type in xpath expr: ${`val`.javaClass.name}")
                    // TODO: Does this mess up any of our other plans?
                    `val`.uncast().getString()!!
                }
            }
        }
