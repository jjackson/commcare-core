...
class XPathWeightedChecklistFunc : XPathFuncExpr {
    ...
    override fun evalBody(
        ...
        return if (args.size == 4 && evaluatedArgs[2] is XPathNodeset && evaluatedArgs[3] is XPathNodeset) {
            val factors = (evaluatedArgs[2] as XPathNodeset).toArgList() as Array<Any>
            val weights = (evaluatedArgs[3] as XPathNodeset).toArgList() as Array<Any>
            if (factors.size != weights.size) {
                throw XPathTypeMismatchException("weighted-checklist: nodesets not same length")
            }
            checklistWeighted(evaluatedArgs[0], evaluatedArgs[1], factors, weights)
        } else {
            checklistWeighted(
                evaluatedArgs[0],
                evaluatedArgs[1],
                FunctionUtils.subsetArgList(evaluatedArgs as Array<Any?>, 2, 2) as Array<Any>,
                FunctionUtils.subsetArgList(evaluatedArgs as Array<Any?>, 3, 2) as Array<Any>
            )
        }
    }
