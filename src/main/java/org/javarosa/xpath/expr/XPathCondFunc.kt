 ...
 */
class XPathCondFunc : XPathFuncExpr {
    ...
    override fun validateArgCount() {
        if (args.size < 3) {
            throw XPathSyntaxException(
                "$name() function requires at least 3 arguments. ${args.size} arguments provided."
            )
        } else if (args.size % 2 != 1) {
            throw XPathSyntaxException(
                "$name() function requires an odd number of arguments. ${args.size} arguments provided."
            )
        }
    }
