...
abstract class XPathFuncExpr : XPathExpression {
    ...
    constructor(
        name: String,
        args: Array<XPathExpression>,
        expectedArgCount: Int,
        evaluateArgsFirst: Boolean
    ) {
        this.name = name
        this.args = args
    ...
    final override fun evalRaw(model: DataInstance<*>?, evalContext: EvaluationContext): Any {
        ...
        val handler: IFunctionHandler? = evalContext.getFunctionHandlers()[name]
    ...
    private fun evaluateArguments(model: DataInstance<*>?, evalContext: EvaluationContext): Array<Any> {
        ...
        return evaluatedArgs as Array<Any>
    }

    protected abstract fun evalBody(
        model: DataInstance<*>?,
    ...
    override fun equals(other: Any?): Boolean {
        if (other is XPathFuncExpr) {
            ...
            if (name != other.name || args.size != other.args.size ||
                name == "uuid" || name == "random"
            ) {
                return false
            }

            @Suppress("UNCHECKED_CAST")
            return ExtUtil.arrayEquals(args as Array<Any?>, other.args as Array<Any?>, false)
        } else {
            return false
        }
    }
