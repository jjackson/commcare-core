...
class XPathPositionFunc : XPathFuncExpr {
            ...
            try {
                ...
                if (expr is XPathLazyNodeset) {
                    throw XPathTypeMismatchException(
                        "Unable to evaluate `position` on ${expr.getUnexpandedRefString()}, which is empty."
                    )
                } else {
                    throw XPathTypeMismatchException(
                        "Unable to evaluate `position` on empty reference in the context of ${evalContext.contextRef}"
                    )
                }
            }
