...
class XPathFormatDateForCalendarFunc : XPathFuncExpr {
    ...
    override fun evalBody(
        ...
        val formatString: String? = if (evaluatedArgs.size > 2) {
            FunctionUtils.toString(evaluatedArgs[2])
        } else {
            null
        }
    ...
    companion object {
        @JvmField
        val NAME = "format-date-for-calendar"
        ...
        private fun formatDateForCalendar(dateObject: Any, calendar: Any, format: String?): String {
            val date: Date = FunctionUtils.expandDateSafe(dateObject) ?: return ""
            return when {
                "ethiopian" == calendar -> CalendarUtils.ConvertToEthiopian(date, format)
                "nepali" == calendar -> CalendarUtils.convertToNepaliString(date, format)
                else -> throw XPathUnsupportedException("Unsupported calendar type: $calendar")
            }
        }
    }
}
