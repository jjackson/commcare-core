            ...
            try {
                if (typedArgs != null) {
                    ...
                    throw XPathTypeMismatchException(
                        "for function '${handler.getName()}'"
                    )
                }
            } catch (ex: XPathArityException) {
                // With static expr's we treat the ArityException as a parse exception to catch it at
            ...
            if (prototype.size == args.size) {
                ...
                for (i in prototype.indices) {
                    ...
                    if (prototype[i].isAssignableFrom(args[i].javaClass)) {
                        ...
                        try {
                            when {
                                prototype[i] == java.lang.Boolean::class.java -> typed[i] = FunctionUtils.toBoolean(args[i])
                                prototype[i] == java.lang.Double::class.java -> typed[i] = FunctionUtils.toNumeric(args[i])
                                prototype[i] == String::class.java -> typed[i] = FunctionUtils.toString(args[i])
                                prototype[i] == Date::class.java -> typed[i] = FunctionUtils.toDate(args[i])
                            }
                        } catch (_: XPathTypeMismatchException) {
                        }
                    }
