...
// non-standard
class XPathSelectedFunc : XPathFuncExpr {
    ...
    companion object {
        ...
        private fun multiSelected(o1: Any, o2: Any): Boolean {
            val unpacked1 = FunctionUtils.unpack(o1)
            if (unpacked1 !is String) {
                throw generateBadArgumentMessage("selected", 1, "multi-select question", unpacked1!!)
            }

            val unpacked2 = FunctionUtils.unpack(o2)
            if (unpacked2 !is String) {
                throw generateBadArgumentMessage(
                    "selected", 2,
                    "single potential value from the list of select options", unpacked2!!
                )
            }

            val s1 = unpacked1
            val s2 = unpacked2.trim()

            return (" $s1 ").contains(" $s2 ")
        }

        private fun generateBadArgumentMessage(
            functionName: String,
            argNumber: Int,
            type: String,
            endValue: Any
            ...
            return XPathException(
                "Bad argument to function '$functionName'. Argument #$argNumber should be a $type, but instead evaluated to: $endValue"
            )
        }
    }
}
