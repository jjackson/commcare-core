...
class XPathStep : Externalizable {
    ...
    fun testStr(): String? {
        return when (test) {
            ...
            TEST_TYPE_PROCESSING_INSTRUCTION ->
                "proc-instr(" + (if (literal == null) "" else "'$literal'") + ")"
            else -> null
        }
    }
    ...
    override fun equals(other: Any?): Boolean {
        if (other is XPathStep) {
            ...
            when (test) {
                TEST_NAME -> if (name != other.name) return false
                TEST_NAMESPACE_WILDCARD -> if (namespace != other.namespace) return false
                TEST_TYPE_PROCESSING_INSTRUCTION -> if (!ExtUtil.equals(literal, other.literal, false)) return false
            }
     ...
     */
    internal fun matchesWith(o: XPathStep?): Boolean {
        if (o != null) {
            // shortcuts for faster evaluation
            if (axis != o.axis
            ...
            when (test) {
                TEST_NAME -> if (o.test != TEST_NAME_WILDCARD && name != o.name) return false
                TEST_NAMESPACE_WILDCARD -> if (namespace != o.namespace) return false
                TEST_TYPE_PROCESSING_INSTRUCTION -> if (!ExtUtil.equals(literal, o.literal, false)) return false
            }
    ...
    override fun hashCode(): Int {
        var code = axis xor test xor
                (name?.hashCode() ?: 0) xor
                (literal?.hashCode() ?: 0) xor
    ...
    companion object {
        ...
        @JvmField
        val XPathStepInterningEnabled = true
