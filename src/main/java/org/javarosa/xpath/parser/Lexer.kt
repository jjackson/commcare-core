...
object Lexer {
    ...
    fun lex(expr: String?): List<Token> {
        if (expr == null) {
            throw XPathSyntaxException("XPath expression is null")
        }
    ...
    private fun badParse(expr: String, i: Int, c: Char) {
        ...
        val postcontext = if (i == expr.length - 1) "" else
            expr.substring(
                Math.min(i + 1, expr.length - 1),
                Math.min(i + CONTEXT_LENGTH, expr.length)
            ).trim() + (if (Math.min(i + CONTEXT_LENGTH, expr.length) != expr.length) "..." else "")

        throw XPathSyntaxException("Couldn't understand the expression starting at this point: " + (preContext + start + postcontext))
    }

    private fun matchNumeric(expr: String, startIndex: Int): Int {
    ...
    private fun matchNCName(expr: String, startIndex: Int): Int {
        var i = startIndex

        while (i < expr.length) {
            val c = expr[i].code
            if (!(isAlpha(c) || c == '_'.code || (i > startIndex && (isDigit(c) || c == '.'.code || c == '-'.code)))) {
                break
            }
            i++
        }
