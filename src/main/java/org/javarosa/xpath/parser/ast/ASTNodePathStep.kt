...
class ASTNodePathStep : ASTNode() {
    ...
    fun getStep(): XPathStep {
        if (nodeTestType == NODE_TEST_TYPE_ABBR_DOT) {
            ...
            step = when (nodeTestType) {
                NODE_TEST_TYPE_QNAME -> XPathStep(axisVal, nodeTestQName!!)
                NODE_TEST_TYPE_WILDCARD -> XPathStep(axisVal, XPathStep.TEST_NAME_WILDCARD)
                NODE_TEST_TYPE_NSWILDCARD -> XPathStep(axisVal, nodeTestNamespace!!)
                NODE_TEST_TYPE_FUNC -> {
                    val funcName = nodeTestFunc!!.name.toString()
                    val type = when (funcName) {
                    ...
                    val s = XPathStep(axisVal, type)
                    if (nodeTestFunc!!.args.isNotEmpty()) {
                        s.literal = (nodeTestFunc!!.args[0] as ASTNodeAbstractExpr).getToken(0)!!.`val` as String
                    }
                    s
                }
            ...
            val preds = Array<XPathExpression>(predicates.size) { i ->
        ...
        fun validateNodeTypeTest(f: ASTNodeFunctionCall): Boolean {
            ...
            return when {
                name == "node" || name == "text" || name == "comment" || name == "processing-instruction" -> {
                    when {
                        f.args.isEmpty() -> true
                        name == "processing-instruction" && f.args.size == 1 -> {
                            val x = f.args[0] as ASTNodeAbstractExpr
                            x.size() == 1 && x.getTokenType(0) == Token.STR
                        }
                        else -> false
                    }
                }
